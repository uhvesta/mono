...
2. Update the `required_status_checks` in GitHub branch protection (Settings → Branches → main).
3. Verify the new check name appears in a real build before the old protection rule is deleted — otherwise the gate silently drops out.
