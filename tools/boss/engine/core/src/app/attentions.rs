...
pub(super) async fn handle_restore_attention_group(ctx: Dispatch, req: FrontendRequest) {
    let Dispatch {
        server_state,
        work_db,
        sink,
    ...
    let FrontendRequest::RestoreAttentionGroup { id } = req else {
        unreachable!()
    };
    match work_db.restore_attention_group(&id) {
        Ok(group) => {
            let members = work_db
                .list_attentions_for_group(&group.id)
                .unwrap_or_default();
