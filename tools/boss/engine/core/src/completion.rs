...
must not be asked to open one",
    ...
    pub async fn recheck_for_pr(&self, execution_id: &str) -> StopOutcome {
        ...
        match self.evaluate_sha_delta_gate(execution_id, &execution).await {
            ...
            ShaDeltaGateOutcome::NoContribution { pr_url } => {
                ...
                // (no PR on the revision's own branch) and the next sweep
                // retries, waiting for a push that moves the head.
                //
                // But a CI-fix revision can legitimately finish WITHOUT
                // moving the head: when the failing required check was a
                // PR-metadata validator (a description / template check), the
                // worker repairs it with `gh pr edit --body` — no commit, no
                // new head SHA. The SHA-delta gate then returns
                // `NoContribution` on every sweep forever, the cold-path
                // detector finds no PR on the revision's own branch, and the
                // execution is stranded in `waiting_human` while its card
                // sits in Doing (issue #1252). `on_stop`'s
                // `try_retire_cleared_blocking_signal` only covers the case
                // where the signal was ALREADY cleared at Stop time; when CI
                // goes green *after* the worker parked, this recovery sweep
                // is the only path that can finalize it.
                //
                // Resolve it against the bound PR's live state: finalize the
                // revision once the parent PR is open with clean CI (the fix
                // landed) or already merged. While CI is still failing or
                // in-flight, fall through and retry on the next sweep.
                if execution.kind == ExecutionKind::RevisionImplementation {
                    if let Some(outcome) = self
                        .finalize_revision_if_bound_pr_healthy(execution_id, &pr_url)
                        .await
                    {
                        return outcome;
                    }
    ...
    /// Recovery for a CI-fix revision that repaired its parent PR with a
    /// PR-metadata-only edit (`gh pr edit --body`, no commit). Such a
    /// revision never moves the bound PR's head SHA, so the SHA-delta gate
    /// reports `NoContribution` on every merge-poller sweep and neither
    /// `on_stop` nor the cold-path detector ever finalizes it — the
    /// execution is stranded in `waiting_human` and its task card sits in
    /// Doing forever (issue #1252).
    ///
    /// Probe the bound parent PR and decide from its live state:
    ...
    ///     failed → return `None` so the caller falls through and the next
    ///     sweep re-probes.
    ///
    /// Returns `Some(outcome)` only when it finalized the execution. This is
    /// race-free against the `ci_watch` retire path: it keys solely on the
    /// bound PR's live CI state, not on the (possibly already-retired)
    /// `ci_remediations` attempt row. Idempotent for revisions that already
    /// finalized via the normal commit-pushing path —
    /// `finalize_pr_transition` returns `AlreadyTerminal` for a non-live
    /// execution.
    async fn finalize_revision_if_bound_pr_healthy(
        &self,
        execution_id: &str,
        bound_pr_url: &str,
    ) -> Option<StopOutcome> {
        let probe = match self.merge_probe.probe(bound_pr_url).await {
            ...
            Err(err) => {
                tracing::debug!(
                    ...
                    bound_pr_url,
                    ?err,
                    "pr-recheck: metadata-only revision bound-PR probe failed; \
        ...
        let target = match &probe.state {
            PrLifecycleState::Open(open) if matches!(open.ci, OpenPrCiStatus::Clean) => {
                tracing::info!(
                    execution_id,
                    bound_pr_url,
                    "pr-recheck: CI-fix revision's bound PR is open with clean CI but its head \
                     never moved (PR-metadata-only fix) — finalizing the revision to in_review \
                     (issue #1252)",
                );
                WorkerPrCompletionTarget::InReview
            }
            PrLifecycleState::Merged => {
                tracing::info!(
                    execution_id,
                    bound_pr_url,
                    "pr-recheck: CI-fix revision's bound PR is already merged — finalizing the \
                     revision to done (issue #1252)",
                );
                WorkerPrCompletionTarget::Done
            }
            // CI still failing / in-flight, or PR closed-unmerged: the fix
            // has not demonstrably landed. Leave the revision parked; the
...
PR #379. PR #379. PR #379. PR #379. PR #379.";
    ...
    // -----------------------------------------------------------
    // PR-metadata-only CI-fix revision recovery (issue #1252).
    //
    // A CI-fix revision that repairs the failing required check by
    // editing only the PR description (`gh pr edit --body`, no commit)
    // never moves the bound PR's head SHA. The SHA-delta gate therefore
    // returns `NoContribution` on every merge-poller sweep, the cold-path
    // detector finds no PR on the revision's own branch, and the
    // execution is stranded in `waiting_human` with its card in Doing.
    // `recheck_for_pr` must finalize it once the bound PR's CI is green.
    // -----------------------------------------------------------

    /// Configurable [`MergeProbe`] returning a fixed lifecycle state for
    /// any PR url. Used to drive the bound-PR-health check.
    ...
    #[tokio::test]
    async fn recheck_finalizes_metadata_only_revision_when_bound_pr_ci_clean() {
        use crate::merge_poller::{OpenPrStatus, PrLifecycleState};

        let workspace = tempdir().unwrap();
        let parent_pr_url = "https://github.com/spinyfin/mono/pull/1009";
        let head = "1111111111111111111111111111111111111111";
        let (db, product_id, revision_id, execution_id) =
            revision_fixture(workspace.path(), parent_pr_url, head);

        // Cold-path detector finds no PR on the revision's own branch
        // (revisions never open their own PR) — the live-engine shape.
        let detector = StubPrDetector::ok(None);
        let cube = Arc::new(StubCubeClient::default());
        let publisher = Arc::new(RecordingPublisher::default());
        let pane = Arc::new(RecordingPaneReleaser::default());
        let probes = Arc::new(RecordingProbeQueuer::default());
        // Head SHA unchanged → SHA-delta gate reports NoContribution.
        let verifier = StubBranchVerifier::ok("boss/exec_parent");
        verifier.set_head_oid(Ok(head.into())).await;
        // The PR-template check went green after the description edit: the
        // bound PR is open with clean CI.
        ...
        assert!(
            ...
            "metadata-only CI-fix revision must finalize to in_review once its bound PR is \
             open with clean CI; got {outcome:?}",
        );

        // The revision task advanced out of Doing to Review. Revision tasks
        // never own a pr_url (the parent chain root is the source of truth),
        // so it stays NULL.
        let revision = db.get_work_item(&revision_id).unwrap();
        match revision {
            WorkItem::Task(t) | WorkItem::Chore(t) => {
                assert_eq!(t.status, "in_review");
                assert!(
                    t.pr_url.is_none(),
                    "revision tasks must not own a pr_url; got {:?}",
                    t.pr_url
                );
        ...
        // Execution finalized — lease released, pane torn down.
        let execution = db.get_execution(&execution_id).unwrap();
        assert_eq!(execution.status, "completed");
        assert!(execution.cube_lease_id.is_none());
        assert_eq!(
            cube.release_calls.lock().await.as_slice(),
            ["lease-1"],
            "the recovery must release the cube lease",
        );
        assert_eq!(
            pane.calls.lock().await.as_slice(),
            [execution_id.as_str()],
            "the recovery must tear down the pane",
        );
        // No probe queued — this is a poller recovery, not an on-Stop nudge.
        assert!(
            probes.snapshot().is_empty(),
            "recovery must NOT queue a worker probe",
        );
        let work_events = publisher.work_events.lock().await.clone();
        assert!(
            work_events.iter().any(|(p, w, _)| p == &product_id && w == &revision_id),
            "work-item invalidation must fire for the revision task",
        );
    }

    #[tokio::test]
    async fn recheck_leaves_metadata_revision_parked_while_bound_pr_ci_failing() {
        ...
        let verifier = StubBranchVerifier::ok("boss/exec_parent");
        verifier.set_head_oid(Ok(head.into())).await;
        // The PR-template check is still red: the fix has not demonstrably
        // landed, so the revision must stay parked.
        ...
        assert_eq!(
            outcome,
            StopOutcome::AwaitingInput,
            "while the bound PR's CI is still failing the revision must stay parked; got {outcome:?}",
        );

        // Revision task untouched — still active (Doing), execution still
        // waiting_human, lease still held.
        let revision = db.get_work_item(&revision_id).unwrap();
        match revision {
