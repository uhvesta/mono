...
fn create_question_assigns_group_short_id_and_grouping_key() {
    ...
    assert_eq!(group.grouping_key, format!("question|{project_id}|doc:docs/x.md"));
...
fn same_grouping_key_reconciles_into_one_group() {
    ...
    let (_a1, g1) = db.create_attention(question(&project_id, "docs/x.md", "Q1")).unwrap();
    let (a2, g2) = db.create_attention(question(&project_id, "docs/x.md", "Q2")).unwrap();
...
fn different_doc_paths_make_separate_groups() {
    let (db, _product, project_id, _task) = fixture();
    let (_a, g1) = db.create_attention(question(&project_id, "docs/x.md", "Q")).unwrap();
    let (_b, g2) = db.create_attention(question(&project_id, "docs/y.md", "Q")).unwrap();
...
fn answering_one_member_moves_group_to_partially_answered() {
    let (db, _product, project_id, _task) = fixture();
    let (a1, _) = db.create_attention(question(&project_id, "docs/x.md", "Q1")).unwrap();
    let (_a2, _) = db.create_attention(question(&project_id, "docs/x.md", "Q2")).unwrap();
...
fn answering_a_question_without_a_value_is_rejected() {
    let (db, _product, project_id, _task) = fixture();
    let (a, _) = db.create_attention(question(&project_id, "docs/x.md", "Q")).unwrap();
...
fn reconcile_bumps_generation_past_a_dismissed_group() {
    let (db, _product, project_id, _task) = fixture();
    let (_a, g0) = db.create_attention(question(&project_id, "docs/x.md", "Q")).unwrap();
...
fn answering_a_member_of_a_dismissed_group_is_rejected() {
    let (db, _product, project_id, _task) = fixture();
    let (a, g) = db.create_attention(question(&project_id, "docs/x.md", "Q")).unwrap();
    db.dismiss_attention(&g.id, None).unwrap();
    let err = db
...
fn adding_a_member_after_a_partial_answer_keeps_partially_answered() {
    ...
    let (_a2, group) = db.create_attention(question(&project_id, "docs/x.md", "Q2")).unwrap();
    assert_eq!(group.state, "partially_answered");
}
...
fn list_applies_kind_and_state_filters() {
    ...
    let (_q, qg) = db.create_attention(question(&project_id, "docs/x.md", "Q")).unwrap();
    let (_f, _fg) = db.create_attention(followup(&task_id, "do it")).unwrap();

    // Default (no state): both open groups are actionable.
    let all = db.list_attention_groups(&product_id, None, None, None, None).unwrap();
    assert_eq!(all.len(), 2);
    ...
    let default_now = db.list_attention_groups(&product_id, None, None, None, None).unwrap();
...
fn multiple_choice_question_requires_choice_options() {
    ...
    let err = db.create_attention(input).unwrap_err();
    assert!(err.to_string().contains("choice_options"), "unexpected error: {err}");
}
...
fn question_from_task(project_id: &str, source_task_id: &str, doc_path: &str, prompt: &str) -> CreateAttentionInput {
    CreateAttentionInput::builder()
        .kind("question")
        .association_project_id(project_id)
        .source_kind("design_doc")
...
fn action_question_with_open_pr_creates_revision() {
    let (db, _product, project_id, task_id) = fixture();
    set_task_pr(&db, &task_id, "https://github.com/spinyfin/mono/pull/42", "in_review");
    let (a, _g) = db
...
fn action_question_with_merged_doc_creates_design_task() {
    let (db, _product, project_id, task_id) = fixture();
    set_task_pr(&db, &task_id, "https://github.com/spinyfin/mono/pull/7", "in_review");
    let (a, _g) = db
        .create_attention(question_from_task(
            &project_id,
            &task_id,
...
fn action_requires_every_member_terminal() {
    let (db, _product, project_id, _task) = fixture();
    let (a1, _g) = db.create_attention(question(&project_id, "docs/x.md", "Q1")).unwrap();
    let (_a2, _g) = db.create_attention(question(&project_id, "docs/x.md", "Q2")).unwrap();
    let g = db
...
fn action_with_skip_unanswered_clears_open_members_then_actions() {
    let (db, _product, project_id, task_id) = fixture();
    set_task_pr(&db, &task_id, "https://github.com/spinyfin/mono/pull/9", "in_review");
    let (a1, _g) = db
...
fn reconcile_followups_dedupe_on_proposed_name() {
    ...
    let batch = || vec![followup(&task_id, "wire retries"), followup(&task_id, "add metrics")];
    let (group, created) = db.reconcile_attentions(batch()).unwrap().unwrap();
