...
private struct ClaudeMonitorTracker {
...
final class TerminalPaneSession: ObservableObject, Identifiable {
    ...
    init(id: Int, launchSpec: TerminalLaunchSpec) {
        self.id = id
        self.title = "Terminal \(id)"
        self.launchSpec = launchSpec
        self.displayTitle = "Claude \(id)"
        self.workingDirectory = launchSpec.workingDirectory
    }

    func setTitle(_ title: String) {
        displayTitle = title.isEmpty ? self.title : title
    }
...
final class TerminalWorkspaceModel: ObservableObject {
    ...
    init() {
        ...
        self.panes = (1...Self.paneCount).map { TerminalPaneSession(id: $0, launchSpec: launchSpec) }
    }
}
